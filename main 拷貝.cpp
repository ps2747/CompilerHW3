...
*/
ParseTreeNode testNode;



pair<Type, string> ParamDecl()
{
	Type type=T_INT;
	string id="a";
	return {type, id};
...
int main(int argc, char *argv[])
{
	...
	list<pair<Type, string > >paramsmit ;
	paramsmit.push_back(ParamDecl());
	Block *currentBlock = new Block (BT_FUNC, T_INT, "main", globalBlock, 0);
	Block *parentBlock = globalBlock;

	currentBlock->asFuncBlock(paramsmit);
	currentBlock->addVariableDecl(T_INT, "b", 0);
	string tempVar = currentBlock->addBinaryExpr("a", BO_SUB, "b");
	currentBlock->addAssignStmt("a", tempVar);
	

	parentBlock = currentBlock;
	currentBlock = new Block(BT_IF, T_VOID,"if", parentBlock, parentBlock->m_tempCounter);
	currentBlock->asIfBlock("slt i32 %17, 9");
	string ifEnd = currentBlock->endIfBlock();

	currentBlock = currentBlock->getParentBlock();
	currentBlock->catLL(ifEnd);

	cout<<currentBlock->endFuncBlock()<<endl;
